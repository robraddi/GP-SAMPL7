...
def write_input_files(dir):
    ...
    for file in files:
        ...
        pairs = pd.read_csv(dir+"microstate_pairs/"+SM_ID+"_microstate_pairs.csv")
        # NOTE: Now get the microtransitions for each SM
        # using the first microstate as the reference
        #microtransitions = GPR.Processing.get_micro_transitions(input_file, column=3, ref="000")
        microtransitions = GPR.Processing.get_micro_transitions_from_pairs(input_file, pairs)
...
# TODO: Make sure that the reference is consistent with Bas.
#write_input_files(dir="Structures/SAMPL6/")
#exit()
#:}}}
...
def get_total_features(path):
    ...
    for file in files:
        ...
        #########################################################
        #NOTE: Testing
        #if int(SM_ID.split("SM")[-1]) <= 2: continue
        #########################################################
...
#get_total_features(path="Structures/SAMPL6/input_files/*/SM*.csv")
#exit()
#:}}}
...
path = "Structures/SAMPL6/input_files/SM*/SM*.csv"
files = GPR.toolbox.get_files(path)
...
def train_for_kernel_parameters(file, training, feats):
    ...
    kernels = []
    for i in layers[1:]:
        kernels += [GPy.kern.Matern52(i, ARD=True)]
...
for i,file in enumerate(files):
    ...
    # NOTE: Making a condition to replace the partial charges that are NAN with extended
    indices = np.where(np.isnan(total_df['AM1BCC partial charge (prot. atom)'].to_numpy(dtype=float)))[0]
    AM1BCC_keys = list(range(4,10))
    Huckel_keys = list(range(16,22))
    if indices != []:
        for x,y in zip(AM1BCC_keys,Huckel_keys):
            x,y = total_df.keys()[x],total_df.keys()[y]
            for ind in indices:
                total_df[x].iloc[[ind]] = total_df[y].iloc[[ind]]
    ###########################################################################
    ...
    df = pd.DataFrame()
    df["posterior"], df["posterior_std"] = posterior, posterior_std
    df["protonated"],df["deprotonated"] = list(total_df["protonated microstate ID"]),list(total_df["deprotonated microstate ID"])
    df["small molecule"] = [SM_ID for i in range(len(posterior))]

    #print(test)
    #fig.savefig(os.path.join(output_path,f"{name}_Posterior_only.png"))

    df.to_pickle(os.path.join(output_path,f"GPy_results.pkl"))
    #print(f"Kernel: {name}")
    if isnotebook():
